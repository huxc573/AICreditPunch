...
- **幂等**：先查今日状态，已签到只打印状态与余额，**不重复领取**，标注「已签到（不重签）」。
...
[09:33:08] [示例账号] 已签到（不重签）；连签1/已签1；本次 +100，余额2,242.18（套餐142.18+奖励2,100）
[09:33:08] WorkBuddy 今日通知已推送，跳过
[09:33:08] ===== Trae =====
[09:33:08] [示例账号] 已签到（不重签）；本次 +150，余额484
[09:33:08] 本次脚本执行完毕。
```
...
| 日志 / 现象 | 含义 / 处理 |
|---|---|
| `已签到（不重签）；…` / `今日通知已推送，跳过` | 正常（幂等 / 同日第二次） |
