...
[示例账号] 今日已签到，本次无需签到；本次 +100，当前积分余额 2,242.18
[示例账号] 积分构成；套餐基础 142.18，平台奖励 2,100
WorkBuddy 今日通知已推送，跳过
本次脚本执行完毕。
```

- **首行 / 末行**是运行起止标记（所有退出路径都有）；中间按 `===== 平台 =====` 分块，
  块内每行 `[账号名] 状态；明细`，与账号无关的平台级提示才写成 `平台 状态；明细`。
- `当前积分余额` 是**真实可用积分**（§2.8）；推送文案 `[平台] 账号名 状态；明细`。
...
| `当前积分余额 N` | 账号真实可用积分 | WorkBuddy：`get-user-resource-summary`；Trae：`total_amount - consumed_amount` |
| `积分构成；…` | 余额构成（为 0 的分项省略） | WorkBuddy 的三条资源包接口 |
...
| 日志 / 现象 | 含义 / 处理 |
|---|---|
| `今日已签到，本次无需签到；…` / `今日通知已推送，跳过` | 正常（幂等 / 同日第二次） |
