...
[示例账号] 本期活动；高校新生攻略 第9期 09-16 ~ 09-29（剩 13 天，进行中），本期连续 1 天，本期已签 1 天
...
- `本期活动` 是 WorkBuddy 的**当期活动口径**（名/期号/窗口 + 本期连续与已签天数）；
...
| `本期活动；…` | 活动名/期号/窗口 + 本期连续与已签天数 | WorkBuddy：`activity_name` / `season` / `start_time` / `end_time` / `active` / `streak_days` / `checkin_dates` |
...
- ⚠️ `streak_days` / `checkin_dates` / `week_checkin_days` 都是**当期活动口径**，换期（如第 8 期 → 第 9 期）
  归零；跨期连续天数服务端不提供。所以「本期连续 1 天」不等于「只签了 1 天」。
