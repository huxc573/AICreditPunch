...
- **幂等**：先查今日状态，已签到只打印状态与余额，**不重复领取**，标注「本次无需签到」。

### 2.5 输出与日志

```text
[26.09.17 09:33:06] 一体化每日签到脚本 v1.8.2 启动
[26.09.17 09:33:06] ===== WorkBuddy =====
[26.09.17 09:33:06] [示例账号] 查询签到状态
[26.09.17 09:33:07] [示例账号] 本期活动；高校新生攻略 第9期 09-16~09-29（剩13天，进行中），本期连签1天，已签1天
[26.09.17 09:33:08] [示例账号] 今日已签到（不重签）；本次 +100，积分余额2,242.18，构成：套餐基础142.18，平台奖励2,100
[26.09.17 09:33:08] WorkBuddy 今日通知已推送，跳过

[26.09.17 09:33:08] ===== Trae =====
[26.09.17 09:33:08] [示例账号] 查询签到状态；device=26982625***
[26.09.17 09:33:08] [示例账号] 今日已签到（不重签）；本次 +150，积分余额484
[26.09.17 09:33:08] 本次脚本执行完毕。
```

- 行首 `[26.09.17 09:33:06]` 是时间戳（两位年 + 点分日期 + 时分秒，`checkin.bat` 的 `:now` 同款）；
  **首行 / 末行**是运行起止标记（所有退出路径都有）；中间按 `===== 平台 =====` 分块、**平台之间空一行**，
  块内每行 `[账号名] 状态；明细`，与账号无关的平台级提示才写成 `平台 状态；明细`。
- `积分余额N` 是**真实可用积分**，后面接 `构成：…`（§2.8）；推送文案 `[平台] 账号名 状态；明细`，不带构成。
- `本期活动` 是 WorkBuddy 的**当期活动口径**（名/期号/窗口 + 本期连签与已签天数）；
  「已签」与「连续」漏签后会分叉 —— 前者是本期内累计，后者要求连着签（§2.8）。
...
| `本期活动；…` | 活动名/期号/窗口 + 本期连签与已签天数 | WorkBuddy：`activity_name` / `season` / `start_time` / `end_time` / `active` / `streak_days` / `checkin_dates` |
| `积分余额N` | 账号真实可用积分 | WorkBuddy：`get-user-resource-summary`；Trae：`total_amount - consumed_amount` |
| `构成：套餐基础N，平台奖励M，购买积分K` | 余额构成，接在结算行尾部 | WorkBuddy 的三条资源包接口 |
...
- ⚠️ `streak_days` / `checkin_dates` / `week_checkin_days` 都是**当期活动口径**，换期（如第 8 期 → 第 9 期）
  归零；跨期连续天数服务端不提供。所以「本期连签1天」不等于「只签了 1 天」。
- `--status-only` 未签到时**只打活动窗口、不打连续天数**（那一刻的 `streak_days` 是签到前的旧值，会少一天）；
  连续与已签天数都在**签到结算后**用当次报文打印 —— 同一次运行里不会打两遍。
...
两个平台**合并成一条消息**：标题 `每日签到完成 09-17 09:34:15`（时间到秒；任一平台失败则为
`每日签到未全部成功 …`），正文一个账号一行、平台之间不留空行、行首用 `[]` 包裹平台名，如
`[WorkBuddy] 熊猫川 签到成功；本次 +100，当前积分余额 1,534.12`。
...
| 日志 / 现象 | 含义 / 处理 |
|---|---|
| `今日已签到（不重签）；…` / `今日通知已推送，跳过` | 正常（幂等 / 同日第二次） |
