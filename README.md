...
- `当前积分余额` 是**真实可用积分**（§2.8）；推送文案 `账号名（平台）：状态；明细`。
...
- 未签到时**只打活动窗口、不打连续天数**（那一刻的 `streak_days` 是签到前的旧值，会少一天）；
  连续与已签天数都在**签到结算后**用当次报文打印。
...
| 日志 / 现象 | 含义 / 处理 |
|---|---|
| `今日已签到，本次无需签到；…` / `今日已推送成功通知，本次静默跳过` | 正常（幂等 / 同日第二次） |
