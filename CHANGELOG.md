...
## [v1.9.1] - 2026-09-20 13:05

### 新增

- **账号并行跑**：同一平台的账号同时跑（默认 4 路，`AICREDIT_CONCURRENCY` 可调 1–8，`1` = 退回串行）。
  13 个账号实测从 ~24s 降到 ~6s。并发只吃网络等待：每个线程的输出先攒进自己的线程缓冲，
  回主线程**按账号顺序**打出来 —— 日志行序与串行跑完全一致，不会谁先返回谁插队；
  同一块里几行的时间戳因此看着一样（本来就是同时出的结果）。
  写 `config.json` 的 Trae 续期仍串行，从工作线程挪回主线程做。

## [v1.9.0] - 2026-09-20 12:43

### 新增

- **收尾一句合计**：一轮跑完，在 `本次脚本执行完毕。` 之前打一句 `合计：13 个账号全部成功，本次 +1,350`，
  有失败写 `合计：13 个账号，12 成功 1 失败，本次 +1,250`。账号数与成功数取本轮实际结果，
  `本次` 是各账号声明值之和（口径同账号行）。通知正文末尾带**同一句**（`flush_notify` 新增 `summary` 参数）。
  账号多到一屏之后，光看账号行看不出「有没有岔子、今天总共进账多少」—— 这句就是给这个用的。
  `--status-only` 不打（它不算签到）。

## [v1.8.3] - 2026-09-18 07:55
...
- **通知正文改紧凑一行式**：一个账号一行、平台之间不留空行、行首 `[平台]`，如
  `[WorkBuddy] 熊猫川 签到成功；本次 +100，积分余额1,534.12`。原为 `熊猫川（WorkBuddy）：…` 且正文带空行。
- **日志精简**：`查询签到状态` 去掉常量回显（原来每账号每轮都打 `；API=<基址>`）；`积分构成` 省略为 0 的分项
  （`套餐基础 0` / `购买积分 0` 由总额就能反推）；同日已推送的提示缩短为 `今日通知已推送，跳过`。
- **日志时间戳改 `[26.09.17 09:33:06]`**（两位年 + 点分日期，原 `[2026-09-17 09:33:06]`）；
  `checkin.bat` 的 `:now`（PowerShell `Get-Date -Format`）同步成同款格式，`--today` 认新前缀。
- **只留结果行，砍掉进度行**：`查询签到状态` / `提交签到` / `签到已受理，回查确认` 不再落盘（含 Trae 的
  `device=…`）—— 每行都有时间戳，卡在哪一步看时间差就知道，出了事由 `状态查询异常` / `签到失败` 行带
  `HTTP/code/reason/requestId` 定位。
- **构成并入结算行**：`积分构成；…` 不再单开一行，改为接在明细尾部 `，构成：套餐基础N，平台奖励M`；
  状态文案 `今日已签到，本次无需签到` → `今日已签到（不重签）`（通知同步）。
- **活动行改平台级**：同平台各账号看到的是同一期活动，`本期活动` 一行整轮只打一次、且打在该平台第一行
  （`WorkBuddy 本期活动；…`，`_ACTIVITY_SHOWN` 按进程去重）；连签与已签天数属于账号，挪进各自的状态行
  （`今日已签到（不重签）；连签2天，已签2天；本次 +100，积分余额1,534.12，构成：…`），通知里不带。
- **推送标题改脚本名 + 版本**：`一体化每日签到脚本 v{VERSION} 26.09.17 10:31:25`（与启动横幅同款，
  失败时尾部加「（未全部成功）」），标题与正文之间不再空行 —— 企业微信应用消息单换行就是换行。
- **`--status-only` 不再写状态、不再推送**：它只查不领，此前却会走 `orchestrate` 把 `success_date_*` 写成当天，
  导致 `--today` 误报「今日已签到」、当天真实签到也不再推送。
- **通知的去向也收成一句、挪到收尾**：原先每个平台各打一行 `{平台} 今日通知已推送，跳过`，夹在平台块中间
  （`WorkBuddy` 那行被下一个 `===== Trae =====` 劈开）—— 通知本来就是合并成一条的，去向也只报一句：
  `已推送签到通知` / `今日通知已推送，跳过`，紧挨在 `本次脚本执行完毕。` 之前；`orchestrate` 不再直接打日志，
  把今天已推过的平台记进 `skipped` 交给 `main` 收尾统一报。
- **推送成功那行只留一句 `已推送签到通知`**：原为 `已推送签到通知；合并 2 条：WorkBuddy，Trae` —— 「合并几条、
  哪几个平台」从正文与上面的平台块就能看出，不必回显；`plan` 里随之没人读的 `label` 字段一并撤掉。
- **首次签到不再单报「未签到」，已签状态压成 `不重签`**：`今日未签到；本次 +100` 那行删掉 —— 紧跟的结算行
  本来就写了 `签到成功；连签3/已签3；本次 +100`，多一行只是噪声（只在 `--status-only` 里保留，它没有结算行）；
  `已签到（不重签）` → `不重签`：「已签到」能从 `连签2/已签2` 与 `本次 +100` 推出来，只留不显然的那个。
- **账号行再压一轮，日期不再每行重复**：可推断的修饰词去掉（`今日` / `天` / `积分` / `套餐基础` / `平台奖励`），
  构成由 `，构成：套餐基础300.96，平台奖励1,700` 改成括号直连 `（套餐300.96+奖励1,700）`；正文时间戳只留
  `[09:33:08]`，日期留在本轮第一行。最长账号行 138 列 → 101 列（-27%），信息一条没少。
...
- **Trae 的 `--status-only` 未签到时一行都不打**：只返回退出码，控制台什么都没有（WorkBuddy 侧有 `今日未签到`），
  单独跑这个命令看不出状态。补上同一行。
- **双击后不释放单实例锁**：`checkin.bat` 把 `:release_lock` 排在 `:pause_if_double_clicked` 之后，
  双击运行、窗口停在「按任意键继续」时锁一直被占着 —— 此后 15 分钟（陈旧锁门槛）内的自动运行会被判成
  「已有签到在跑」直接跳过。改为**先放锁再等按键**：那时日志已拼接完成，锁已无保护对象。
- **运行收尾的空行位置错了，日志块之间被粘住**：`log("")` 原本排在 `if first_today:` 之前，「当日首次签到，
  正在用记事本打开日志：…」那行落在空行之外 —— 它粘在上一轮运行块的尾部，下一轮运行头紧贴着它，两轮之间
  没有空行（`--today` 的「最近一次运行」也据此切块）。改为空行压在本轮所有输出之后。
