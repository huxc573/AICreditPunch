...
- **开机 / 登录时「Checking the network ...」要等很久**：旧探测用 PowerShell 的
  `TcpClient.Connect` 且**没有超时**，主机不可达时一次探测就卡在 SYN 重试约 20 秒，
  24 次循环实际能拖到十几分钟。现在改用系统自带的 `ping -n 1 -w 1000` 快探测（1 秒内
  必回，无解释器启动开销），通了就停；连续 12 次不通再用 Python 做一次 **3 秒硬超时**
  的 443 握手复核（防止「ICMP 被拦、网其实是通的」），仍不通就记 WARN 继续跑。
  等待总预算由「名义 120 秒、实际可达数分钟」降到约 40 秒。

- **手动运行 `checkin.bat` 窗口只剩一行 `Checking the network ...`**：运行结果一直只写
  进临时日志，控制台只在最后用 `chcp 65001` + `type` 回显一次；实测在真实 conhost 里
  切换代码页会把已打印的内容和回显块一起清掉，所以窗口里几乎什么都不剩。现在改成
  **双输出**：`checkin.py` 把每行实时打到控制台，同时按 `ACP_RUN_LOG` 追加一份临时
  副本，bat 结束后再前置到日志顶端 —— 控制台与日志是同一份文本，全程不切代码页；
  `--auto` 运行时额外置 `ACP_QUIET=1`，控制台仍然完全静默。
