...
- **构成并入结算行、平台之间空一行**：`积分构成；…` 不再单开一行，改为接在明细尾部 `，构成：套餐基础N，平台奖励M`；
  状态文案 `今日已签到，本次无需签到` → `今日已签到（不重签）`（通知同步）；`===== 平台 =====` 之前留空行，
  `_log_blocks` 相应不再把运行内的空行当运行边界（否则 `--today` 的「最近一次运行」只显示第一个平台）。
