...
## [未发布] - 2026-09-17 10:05

### 变更

- **两个平台的通知合并成一条**：原先 WorkBuddy / Trae 各发一条（同日最多 2 条），改为两平台跑完后统一发一条。
  `orchestrate` 只判定「该平台是否要通知」并把内容放进计划，`flush_notify` 负责合并发送与写标记。
- **通知正文改紧凑一行式**：一个账号一行、平台之间不留空行，行首 `[平台]`（如
  `[WorkBuddy] 熊猫川 签到成功；本次 +100，当前积分余额 1,534.12`）；标题带时间且到秒
  （`每日签到完成 09-17 09:34:15`）。原为 `熊猫川（WorkBuddy）：…`、标题只到日。
- **`--status-only` 不再写状态、不再推送**：它只查不领，此前却会走 `orchestrate` 把 `success_date_*` 写成当天，
  导致 `--today` 误报「今日已签到」、当天真实签到也不再推送。

### 修复

- **日志出现截断乱码行 / 重复行**：两个实例并发运行（如连点两次 bat）时，`:publish_block` 用 `copy /b` + `move`
  重写整个日志，另一个实例会读到写了一半的文件 —— 09-16 21:30 那次就留下了一行丢掉行首、含非法 UTF-8 字节的记录，
  同时出现成对的重复行。改为 bat 用 `mkdir` 锁目录做单实例互斥（拿不到锁直接退出，另一个实例正在做同样的签到，
  不会漏签；超过 15 分钟的陈旧锁视为被强杀的残留，自动清掉重试），临时日志文件名也加上双 `%RANDOM%`。
- 锁判定用 `if not errorlevel 1`（`mkdir` 成功才算拿到锁）—— 用 `if exist` 判断会把「目录本来就存在」误判成拿到锁。
